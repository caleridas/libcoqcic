...
visit_transform(
	...
	return input.visit(
		overload {
			[&](const constr_local& local) -> std::optional<constr_t> {
				return visitor.handle_local(local.name(), local.index());
			},
			[&](const constr_global& global) -> std::optional<constr_t> {
				return visitor.handle_global(global.name());
			},
			[&](const constr_builtin& builtin) -> std::optional<constr_t> {
				return visitor.handle_builtin(builtin.name());
			},
			[&](const constr_product& product) -> std::optional<constr_t> {
				...
				if (result) {
					...
					return constr_product::create(std::move(args), restype);
				} else {
					return {};
				}
			},
			[&](const constr_lambda& lambda) -> std::optional<constr_t> {
				std::vector<formal_arg_t> args;
				bool changed = false;
				...
				if (result) {
					...
					return constr_lambda::create(std::move(args), body);
				} else {
					return {};
				}
			},
			[&](const constr_let& let) -> std::optional<constr_t> {
			...
			[&](const constr_apply& apply) -> std::optional<constr_t> {
				...
				if (result) {
					...
					return constr_apply::create(fn, std::move(args));
				} else {
					return {};
				}
			},
			[&](const constr_cast& cast) -> std::optional<constr_t> {
			...
			[&](const constr_match& match) -> std::optional<constr_t> {
				auto maybe_arg = visit_transform(match.arg(), visitor);
				const auto& arg = maybe_arg ? *maybe_arg : match.arg();
				...
				auto maybe_casetype = visit_transform(match.casetype(), visitor);
				const auto& casetype = maybe_casetype ? *maybe_casetype : match.casetype();
				visitor.pop_local();
			...
			[&](const constr_fix& fix) -> std::optional<constr_t> {
